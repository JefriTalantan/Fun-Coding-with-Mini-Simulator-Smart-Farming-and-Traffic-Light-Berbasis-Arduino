...
// Access Point (AP) Mode Configuration - Fallback when WiFi connection fails
#define AP_MODE_ENABLED true            // Enable access point mode as fallback
#define AP_SSID "SmartFarming-ESP32"    // Name of the access point network
#define AP_PASSWORD "smartfarm123"      // Password for the access point (min 8 chars)
#define AP_CHANNEL 1                    // WiFi channel for access point (1-13)
#define AP_MAX_CONNECTIONS 4            // Maximum clients that can connect to AP
#define AP_IP IPAddress(192, 168, 4, 1) // IP address of the ESP32 in AP mode
#define AP_GATEWAY AP_IP                // Gateway (same as AP IP)
...
// DHT Auto-Disable Settings
#define DHT_MAX_CONSECUTIVE_ERRORS 3    // Max consecutive DHT failures before disabling
#define DHT_HEALTH_CHECK_INTERVAL 60000 // Check DHT health when disabled (1 minute)
#define DHT_RECOVERY_ATTEMPTS 3         // Max recovery attempts when re-enabling DHT
...
/*
 * EEPROM SETTINGS:
 * The system uses EEPROM to save critical data across reboots and power failures.
 * This ensures consistent operation and prevents loss of important settings.
 */

// EEPROM Configuration
#define EEPROM_ENABLED true             // Enable EEPROM functionality
#define EEPROM_SIZE 512                 // Total EEPROM size in bytes
#define EEPROM_START_ADDRESS 0          // Starting address for our data

// EEPROM Data Structure Version (increment when changing data structure)
#define EEPROM_VERSION 1                // Current EEPROM data version

// EEPROM Save Triggers
#define EEPROM_SAVE_INTERVAL 300000      // Auto-save interval (5 minutes)
#define EEPROM_SAVE_ON_CHANGE true      // Save immediately when values change
#define EEPROM_SAVE_ON_SHUTDOWN true    // Save on emergency stop or shutdown

// EEPROM Data Sections (addresses within EEPROM)
#define EEPROM_VERSION_ADDR 0           // 1 byte - data structure version
#define EEPROM_THRESHOLD_ADDR 1         // 2 bytes - soil moisture threshold
#define EEPROM_DAILY_COUNT_ADDR 3       // 2 bytes - daily irrigation count
#define EEPROM_LAST_IRRIGATION_ADDR 5   // 4 bytes - timestamp of last irrigation
#define EEPROM_DHT_ENABLED_ADDR 9       // 1 byte - DHT sensor enabled status
#define EEPROM_DHT_ERROR_COUNT_ADDR 10  // 2 bytes - DHT error count
#define EEPROM_SYSTEM_FLAGS_ADDR 12     // 1 byte - system status flags
#define EEPROM_CALIBRATION_ADDR 13      // Calibration data (future use)
#define EEPROM_CHECKSUM_ADDR 100        // 2 bytes - data integrity checksum
