...
struct SystemState {
  ...
  bool apModeEnabled = false;         // True when in access point mode
  bool apModeActive = false;          // True when AP is actually running
  int apConnectedClients = 0;         // Number of clients connected to AP
  String currentMode = "OFFLINE";     // Current operating mode: "OFFLINE", "ONLINE", "AP_MODE"
  String apIpAddress = "";            // IP address when in AP mode
...
void initializeSystem() {
  ...
  // Initialize AP-specific variables
  systemState.apModeEnabled = false;
  systemState.apModeActive = false;
  systemState.apConnectedClients = 0;
  systemState.currentMode = "OFFLINE";
  systemState.apIpAddress = "";
...
void initializeWiFi() {
  ...
  // First, try to connect to the configured WiFi network
  if (strlen(WIFI_SSID) > 0 && strcmp(WIFI_SSID, "YOUR_WIFI_SSID") != 0) {
    #if SERIAL_OUTPUT_ENABLED
      Serial.println("Attempting to connect to WiFi network: " + String(WIFI_SSID));
    #endif
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      systemState.currentMode = "ONLINE";
      systemState.apModeEnabled = false;
      systemState.apModeActive = false;
      #if SERIAL_OUTPUT_ENABLED
        Serial.println();
        ...
        Serial.println("System operating in ONLINE mode");
      #endif
    } else {
      #if SERIAL_OUTPUT_ENABLED
        Serial.println();
    ...
    #if SERIAL_OUTPUT_ENABLED
      Serial.println("No WiFi credentials configured - skipping WiFi connection");
    #endif
  }

  // If WiFi connection failed or no credentials provided, start Access Point mode
  if (!wifiConnected && AP_MODE_ENABLED) {
    #if SERIAL_OUTPUT_ENABLED
    ...
    systemState.apModeEnabled = true;
    systemState.apModeActive = true;
    systemState.currentMode = "AP_MODE";
    systemState.apIpAddress = AP_IP.toString();

    #if SERIAL_OUTPUT_ENABLED
      Serial.println("Access Point mode activated!");
      Serial.println("SSID: " + String(AP_SSID));
      Serial.println("Password: " + String(AP_PASSWORD));
      Serial.println("IP Address: " + systemState.apIpAddress);
      Serial.println("System operating in LOCAL/AP mode");
      Serial.println("Connect to '" + String(AP_SSID) + "' to access the web interface");
    #endif
  } else if (!wifiConnected) {
    systemState.wifiConnected = false;
    systemState.currentMode = "OFFLINE";
    #if SERIAL_OUTPUT_ENABLED
      Serial.println("System operating in OFFLINE mode (no connectivity)");
    #endif
  }
}
...
void checkAccessPointStatus() {
  if (systemState.apModeActive) {
    // Update the number of connected clients
    systemState.apConnectedClients = WiFi.softAPgetStationNum();

    #if SERIAL_OUTPUT_ENABLED && DEBUG_MODE
      static int lastClientCount = -1;
      if (systemState.apConnectedClients != lastClientCount) {
        Serial.println("AP Mode - Connected clients: " + String(systemState.apConnectedClients));
        lastClientCount = systemState.apConnectedClients;
      }
    #endif
  }
}
...
void displayWiFiStatus() {
    ...
    if (systemState.currentMode == "ONLINE") {
      lcd.print("WiFi Connected");
      delay(DISPLAY_SCROLL_DELAY);
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print("IP: " + WiFi.localIP().toString());
    } else if (systemState.currentMode == "AP_MODE") {
      ...
      lcd.setCursor(0, 1);
      lcd.print("IP: " + systemState.apIpAddress);
    } else {
...
void handleRoot() {
  ...
  String modeColor = systemState.currentMode == "ONLINE" ? "ok" : (systemState.currentMode == "AP_MODE" ? "warning" : "error");
  html += "<div class='card' style='background: #e8f4f8; border-left: 4px solid " +
          String(modeColor == "ok" ? "green" : (modeColor == "warning" ? "orange" : "red")) + ";'>";
  html += "<h3>Operating Mode: <span class='status " + modeColor + "'>" + systemState.currentMode + "</span></h3>";
  if (systemState.currentMode == "AP_MODE") {
    html += "<p><strong>Local Access Point Mode</strong></p>";
    html += "<p>Connect to WiFi network: <strong>" + String(AP_SSID) + "</strong></p>";
    html += "<p>Password: <strong>" + String(AP_PASSWORD) + "</strong></p>";
  } else if (systemState.currentMode == "ONLINE") {
    html += "<p><strong>Connected to WiFi: " + String(WIFI_SSID) + "</strong></p>";
  }
  html += "</div>";
  ...
  // Connection Status
  if (systemState.currentMode == "ONLINE") {
    html += "<p>WiFi Status: <span class='status ok'>CONNECTED</span></p>";
    html += "<p>IP Address: " + WiFi.localIP().toString() + "</p>";
  } else if (systemState.currentMode == "AP_MODE") {
    html += "<p>Access Point: <span class='status warning'>ACTIVE</span></p>";
    html += "<p>AP IP Address: " + systemState.apIpAddress + "</p>";
    html += "<p>Connected Clients: " + String(systemState.apConnectedClients) + "</p>";
  } else {
    html += "<p>Network Status: <span class='status error'>OFFLINE</span></p>";
  }

  html += "</div>";
  
  // Cloud Services Status (only show when online)
  if (systemState.currentMode == "ONLINE") {
    html += "<div class='card'>";
  ...
  // Network Information (different content based on mode)
  html += "<div class='card'>";
  ...
  if (systemState.currentMode == "ONLINE") {
    ...
    html += "<p><strong>Access Point Mode</strong></p>";
    html += "<p>Network Name (SSID): " + String(AP_SSID) + "</p>";
    html += "<p>IP Address: " + systemState.apIpAddress + "</p>";
    html += "<p>Connected Devices: " + String(systemState.apConnectedClients) + "/" + String(AP_MAX_CONNECTIONS) + "</p>";
    html += "<p><em>Connect to this WiFi network to access the interface</em></p>";
  } else {
    html += "<p><strong>Offline Mode</strong></p>";
    html += "<p>No network connectivity available</p>";
    html += "<p>Configure WiFi settings to enable online features</p>";
  }
...
void updateLEDs() {
  ...
  // - Solid: Online mode (connected to WiFi)
  // - Fast blink: AP mode (access point active)
  // - Slow blink: Offline mode (no connectivity)
  if (systemState.currentMode == "ONLINE") {
    digitalWrite(LED_BLUE_PIN, HIGH);  // Solid blue for online mode
  } else if (systemState.currentMode == "AP_MODE") {
    // Fast blink for AP mode (200ms on/off)
    static unsigned long lastBlueBlink = 0;
    if (currentTime - lastBlueBlink >= 200) {
      digitalWrite(LED_BLUE_PIN, !digitalRead(LED_BLUE_PIN));
      lastBlueBlink = currentTime;
    }
  } else {
    // Slow blink for offline mode (1000ms on/off)
    static unsigned long lastBlueBlink = 0;
    if (currentTime - lastBlueBlink >= 1000) {
      digitalWrite(LED_BLUE_PIN, !digitalRead(LED_BLUE_PIN));
      lastBlueBlink = currentTime;
    }
  }
...
void performHeartbeat() {
    ...
    Serial.println("  Operating Mode: " + systemState.currentMode);
    if (systemState.currentMode == "ONLINE") {
      Serial.println("  WiFi Status: " + String(systemState.wifiConnected ? "CONNECTED" : "DISCONNECTED"));
    } else if (systemState.currentMode == "AP_MODE") {
      Serial.println("  AP Status: ACTIVE (SSID: " + String(AP_SSID) + ")");
      Serial.println("  Connected Clients: " + String(systemState.apConnectedClients) + "/" + String(AP_MAX_CONNECTIONS));
    } else {
      Serial.println("  Network Status: OFFLINE");
    }
    #if DHT_ENABLED
...
String getSystemStatusJSON() {
  ...
  doc["currentMode"] = systemState.currentMode;
  doc["apModeActive"] = systemState.apModeActive;
  doc["apConnectedClients"] = systemState.apConnectedClients;
  doc["apIpAddress"] = systemState.apIpAddress;
  #if DHT_ENABLED
  doc["dhtEnabled"] = systemState.dhtSensorEnabled;
  doc["dhtErrorCount"] = systemState.dhtErrorCount;
  doc["dhtStatus"] = systemState.dhtStatus;
  #else
  doc["dhtEnabled"] = false;
  doc["dhtErrorCount"] = 0;
  doc["dhtStatus"] = "DISABLED";
  #endif
  #if IOT_SERVICES_ENABLED && ADAFRUIT_IO_ENABLED
...
void displayAllInfo() {
    ...
    // Line 3: System Status and Mode
    lcd.setCursor(0, 2);
    String systemStatus = "Status:" + String(systemState.systemOK ? "OK" : "ERR") + " Mode:" + systemState.currentMode;
    #if DHT_ENABLED
      systemStatus += " DHT:" + (systemState.dhtSensorEnabled ? "ON" : "OFF");
    #endif
...
void outputToSerial() {
    ...
    if (currentTime - lastSerialOutput >= DISPLAY_UPDATE_INTERVAL) {
      String networkStatus = systemState.currentMode == "ONLINE" ? "WiFi:OK" :
                            (systemState.currentMode == "AP_MODE" ? "AP:ON" : "OFFLINE");
      Serial.println("Status: " + String(systemState.systemOK ? "OK" : "ERROR") +
                    " | Soil: " + String(systemState.soilMoisturePercent) + "%" +
                    " | Pump: " + String(systemState.pumpActive ? "ON" : "OFF") +
